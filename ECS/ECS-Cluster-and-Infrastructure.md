...
- **Introduction**
  - ECS Cluster-level encryption is a security feature in Amazon ECS (Elastic Container Service) that automatically encrypts all the data in your cluster using a KMS key
  - Only authorized users/apps with permission can use this key to read or modify the encrypted data.
  - Encryption Protects your containers, tasks, and secrets from hackers or leaks.
  - ECS Supports Two Types of Storage.
    - Ephemeral Storage (Temporary)
    - Managed Storage (Persistent)
    
   - **Ephemeral Storage (Temporary)**
     - Nature: Temporary disk space that exists only during task execution
     - Supported Launch type: AWS Fargate
     - Capacity: Default 20GB (configurable up to 200GB)
     - Encryption: Automatically encrypted when cluster-level encryption is enabled
     - Best For
       - Temporary processing files
       - Application cache
       - Short-lived logs
     - Lifecycle: Data vanishes when task stops (like RAM memory)
    
   - **Managed Storage**
     - Types
       - EFS (Elastic File System) - shared network storage
       - EBS (Elastic Block Store) - volume storage (more common with EC2)
     - Nature: Permanent storage that persists beyond task lifetime
     - Supported Launch type
       - AWS Fargate
       - EC2
     - Encryption
       - EFS: Always encrypted by default with KMS
       - EBS: Optional encryption
     - Best For
       - Databases
       - User uploads
       - Configuration files
     - Lifecycle: Persists beyond task lifetime

   - **Key Point**
     - ECS cluster-level encryption (for Secrets encryption using KMS) must be enabled at the time of cluster creation
     - There is no option to turn it on after the cluster is already created
